...
import com.lotus.learn.exception.BusinessException;
...
public class ExceptionAdvisor implements ThrowsAdvice {
	private ILogSV logSV;
	
	public void afterThrowing(Method method, Object[] args, Object target, Exception ex) throws Throwable {
		try {
			
		CoreProcProcessDefinition coreProcProcessDefinition = new CoreProcProcessDefinition();
		coreProcProcessDefinition.setProcessDefinitionId("sdf12");
		logSV.saveProcDef(coreProcProcessDefinition);
		} catch (Exception e) {
			e.printStackTrace();
		}
		// 在后台中输出错误异常异常信息，通过log4j输出。
		Log log = LogFactory.getLog(ExceptionAdvisor.class);
		log.info("**************************************************************");
	...
	public ILogSV getLogSV() {
		return logSV;
	}

	public void setLogSV(ILogSV logSV) {
		this.logSV = logSV;
	}
}
...
(function($) {
	...
	$.tele={
		layout:function(){
			...
			var hdHeight= $("#Tele_hd").height();
			var ftHeight= $("#Tele_ft").height();
			$("#tele_frame,#tele_wrap").css({top:hdHeight}).height(pageH-hdHeight-ftHeight);
			if(pageW<500){
				$("#tele_frame,#tele_wrap").width("500");
				$("#Tele_hd").width("500");
				$("#Tele_ft").width("500");
			}else if(pageW>500){
				$("#tele_frame,#tele_wrap").width(pageW);
				$("#Tele_hd").width(pageW);
				$("#Tele_ft").width(pageW);
			}
		},
		layoutCheck:function(){
			$("#hideleft_layout").hover(function(){
				$(this).addClass("hideleft_layout_hover");						 
			},function(){
			    $(this).removeClass("hideleft_layout_hover");												  
			});
			$("#hideleft_layout").bind("click",function(){
				$(this).css({display:"none"});
				$("#showleft_layout").css({display:"block"});
				$("#tele_layout").layout('collapse','west');
			});
			$("#showleft_layout").hover(function(){
				$(this).addClass("showleft_layout_hover");						 
			},function(){
			    $(this).removeClass("showleft_layout_hover");												  
			});
			$("#showleft_layout").bind("click",function(){
				$("#tele_layout").layout('expand','west');
				$(this).css({display:"none"});
				setTimeout("$('#hideleft_layout').css({display:'block'})",600);
			});
			
		},
		menuList:function(menuId,mueulistid,showCid,hideCid){
				$(menuId+" .tabs .tabs-title").each(function(){
					var tMenuList = $(this).text();
					//var teleMenuIndex = $(this).index();
					$(mueulistid).find("ul").append("<li class='tele_menu_list_l'>"+tMenuList+"</li>");
					$(".tele_menu_list_l").hover(function(){
						$(this).addClass("tele_menu_list_lh");
					},function(){
						$(this).removeClass("tele_menu_list_lh");
					});
				});
				$(mueulistid+" li").bind("click",function(){
					var teleMenuIndex = $(this).index();
					$(menuId).tabs("select",teleMenuIndex);
					hideDom();
				});
				$(showCid).click(function(){
						showDom();
				});
				$(hideCid).click(function(){
						hideDom();
				});
				function showDom(){
					$(mueulistid).fadeIn();
					//var idom = $("iframe").contents();
					setTimeout(function(){$.EX.addEvent.call(document,'click',hideDom);});//这里遗留了一个点击元素以外的iframe没法触发click方法的问题
					//$(document).trigger("click",hideDom);
					//setTimeout(function(){$.EX.addEvent.call(idom,'click',hideDom);});
					$(showCid).hide("fast");
					$(hideCid).show("fast"); 
				}
				/*$(window.frames["iframeDom"].document).click(function(){
						alert(1111);
				});
				$("iframe").contents().click(function(){
						alert(1111);
				});
				$(frames['iframeDom'].document).click(function(){
						alert(1111);
				});
				...
				function hideDom(){
					$(mueulistid).fadeOut();
					//var idom = $("iframe").contents();
					$.EX.removeEvent.call(document,'click',hideDom);
					//$(document).trigger("click",hideDom);
					//$.EX.removeEvent.call(idom,'click',hideDom);
					$(hideCid).hide("fast");
					$(showCid).show("fast"); 
				}
		},
		layout360Check:function(){
			$("#hideViewLayout").bind("click",function(){
				$(this).css({display:"none"});
				$("#showViewLayout").css({display:"block"});
				$("#tele_view").layout('collapse','north');
			});
			$("#showViewLayout").bind("click",function(){
				$("#tele_view").layout('expand','north');
				$(this).css({display:"none"});
				setTimeout("$('#hideViewLayout').css({display:'block'})",600);
			});

...
if (dts[15] == "Dept") {
    if (dts[14] == "True") {
        js_include(itpSelectPath + "lang/deptmen.js");
    }
    else {
        js_include(itpSelectPath + "lang/deptmcn.js");
    }
}
else if (dts[15] == "Employee") {
    if (dts[14] == "True") {
        js_include(itpSelectPath + "lang/employeemen.js");
    }
    else {
        js_include(itpSelectPath + "lang/employeemcn.js");
    }
}
else {
    if (dts[14] == "True") {
        js_include(itpSelectPath + "lang/men.js");
    }
}
if (dts[17] != "") {
    js_include(dts[17]);
...
ZTEDeptSelectItem.searchEmployee = function() {
    ...
    try {
        ...
        document.all.ListBoxLeft.innerHTML = ""; //清空左边列表中所有数据
...
try {
        ...
        var dts = window.dialogArguments; //传参，及传引用句柄
        //var ctlName = dts[1];
        var k = dts[10]; //= dts[0].document.getElementById(ctlName.id + "_input");
        var v = dts[11];//dts[0].document.getElementById(ctlName.id + "_value");

        //if (ctlName.MulitSelect == "False") {
        if (dts[3] == "False") {
            document.all.btnMoveAllRight.style.display = "none";
            document.all.btnMoveAllLeft.style.display = "none";
        }
...
ZTEDeptSelectItem.addRightList = function(dk, dv) {
    ...
    for (var i = 0; i < Arrk.length; i++) {
        if (Arrv[i]) {
            ZTEDeptSelectItem.addOption(document.all.ListBoxRight, Arrk[i], Arrv[i]);
        }
    }
}
...
ZTEDeptSelectItem.addList = function(dt) {
    try {
        ...
        else if (dt.count == 2) //如果查询结果为1条数据，则直接添加到右边列表
        {
            ...
            ZTEDeptSelectItem.addOption(document.all.ListBoxRight, dt.getValue(1, 0), dt.getValue(1, 1));
        ...
        else if (dt.count > 2) //如果查询结果为多条数据，则添加到左边列表
        {
            ...
            for (var i = 1; i < dt.count; i++) {
                ...
                if (i > (MaxQueryDeptNumber - 1))  //如果i值大于配置文件中限制的数量则不再添加到列表框中
                {
                    ...
                    ZTEDeptSelectItem.addOption(document.all.ListBoxLeft, dt.getValue(i, 0), dt.getValue(i, 1));
                }
            }
            if (countKey > dt.count)  //检查输入的关键字与查到的结果数是否一致，不一致提示
            {
...
ZTEDeptSelectItem.addOption = function(ctlList, strText, strValue) {
    ...
    if (document.all.ListBoxRight.length > 0) {
        $("#btnOK").attr("disabled", "");
    }
    else {
...
ZTEDeptSelectItem.moveOption = function(srcList, dstList) {
    ...
    if (dstList == document.all.ListBoxRight
		  && dts[3] == "False") {
        for (var i = 0; i < document.all.ListBoxRight.length; i++) {
            document.all.ListBoxRight[i].selected = true;
        }
        ZTEDeptSelectItem.moveOption(document.all.ListBoxRight, document.all.ListBoxLeft);
    }
    ...
    else if (srcList.selectedIndex != -1) //如果源列表有项目，并且选择了项目，则进行移动处理
    {
        ...
        for (var i = 0; i < srcList.options.length; i++) //对列表项目进行迭代
        {
		    ...
		    && document.all.ListBoxRight.length >= 1) {
                break;
            }
...
{		
	//循环选中左边列表所有项目
	for(var i = 0;i < document.all.ListBoxLeft.length; i++)
	{
		document.all.ListBoxLeft[i].selected = true;
	}
	ZTEDeptSelectItem.moveOption(document.all.ListBoxLeft,document.all.ListBoxRight);
}
...
{
	document.all.lblLeftCount.innerText  = document.all.ListBoxLeft.length; //左列表统计
	document.all.lblRightCount.innerText = document.all.ListBoxRight.length; //右列表统计
}
...
ZTEDeptSelectItem.setMessage = function(str) {
    alert(str);
    //document.all.lblMessage.innerHTML = str;
}
    ...
    if (document.all.ListBoxRight.length > 0) {
        for (var i = 0; i < document.all.ListBoxRight.length; i++) {
            if (i > (MaxSelectedDeptNumber - 1))   //如果大于配置文件中要求的最大数目减1，则不添加到DataTable中
            {
                ...
                strk += document.all.ListBoxRight[i].text;
                strv += document.all.ListBoxRight[i].value;
            }
        }
    }
...
{
    if (event.keyCode == 13)//keyCode == 9
    {
        ZTEDeptSelectItem.searchEmployee();
        event.returnValue = false;
    }	  
}
